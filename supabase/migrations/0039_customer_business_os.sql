-- Phase 5 — Customer + Business Operating System (additive)
-- Reuses profiles, admin_notes, profiles.tags, transactions, enrollments, product_events.

-- Managed tag labels (membership remains on profiles.tags text[])
...
-- Saved rule-based customer segments
...
-- Product bundles (references existing courses — no duplicate product rows)
...
-- Helpful indexes for CRM filters / revenue (idempotent)
...
-- Optional: extend automation_trigger enum for purchase (idempotent)
do $$
begin
  if not exists (
