-- Sales Page Phase 1 (additive only — no destructive changes to existing tables)
...
create index if not exists sales_pages_status_idx
  on public.sales_pages (status);
create index if not exists sales_pages_course_id_idx
  on public.sales_pages (course_id);
...
create index if not exists sales_page_assets_page_idx
  on public.sales_page_assets (sales_page_id);
create index if not exists sales_page_assets_course_idx
  on public.sales_page_assets (course_id);
create index if not exists sales_page_assets_checksum_idx
  on public.sales_page_assets (checksum)
...
create table if not exists public.sales_page_imports (
  ...
  constraint sales_page_imports_source_type_check check (
...
-- Public can read published sales page rows only (schema for landing)
...
-- Public can read active assets that belong to a published sales page
