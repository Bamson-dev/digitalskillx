...
--
-- HOW TO USE:
--   1. Run this ENTIRE file from the TOP (starts at section 0 — do not skip to the bottom).
...
-- ── 5. Platform secrets (0009 + integration columns) ──────────────────────────
...
alter table public.course_categories
  add column if not exists template_key text check (
    template_key is null or template_key in ('gold_charcoal', 'navy_ribbon', 'green_gold')
  );

alter table public.courses
  add column if not exists certificate_template_override text check (
...
-- ── 9. Course resources ordering (0016) ─────────────────────────────────────
